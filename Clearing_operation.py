...
class ClearingOperation:
    def __init__(self, win):
        ...
        self.terrain_type_lbl = Label(win, text='Terrain:')
        self.terrain_type_cbx = Combobox(win, values=('Urban', 'City', 'Instalation'), width=10)
        self.terrain_type_cbx.set('Urban')
        self.efficiency_lbl = Label(win, text='Efficiency:')
        self.efficiency_cbx = Combobox(win, values=('1', '2', '3', '4', '5', '6', '7', '8'), width=4)
        self.efficiency_cbx.set('1')
        self.clearing_number_lbl = Label(win, text='Clearing:')
        self.clearing_number_cbx = Combobox(win, values=('3', '4', '5', '6'), width=4)
        self.clearing_number_cbx.set('3')
        self.stack_points_lbl = Label(win, text='Stack points:')
        self.stack_points_ent = Entry(width=7)
        self.stack_points_ent.insert(END, '0')
        self.installation_per_hex = Combobox(win, values=('0', '1', '2', '3'), width=4)
        self.installation_per_hex.set('0')
        self.clearing_btn = Button(win, text='Clearing', command=self.clearing)
        self.clearing_d10_lbl = Label(win, text='d10:')
        self.clearing_d10_ent = Entry(width=7)
        self.clearing_result_lbl = Label(win, text='Clearing result:')
        self.clearing_result_ent = Entry(width=7)
        self.loss_result_lbl = Label(win, text='Loss')
        self.loss_result_ent = Entry(width=7)

        self.paradrop = BooleanVar()
        self.paradrop.set(False)
        self.paradrop_chk = Checkbutton(win, text='Clearing from paradrop', variable=self.paradrop)

        self.combined_arms = BooleanVar()
        self.combined_arms.set(False)
        self.combined_arms_chk = Checkbutton(win, text='Leg and mechanized/armored', variable=self.combined_arms)
...
window = Tk()
mywin = ClearingOperation(window)
window.title('Clearing Operation')
window.geometry("400x270+10+10")
window.mainloop()
